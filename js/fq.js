        ...
        var foursquareApi = {
            ...
            getPhotos: function(venueAlbums){
                var heightDimension = 400;
                var widthDimension = 400;
                ...
                $.each(venueAlbums, function(index, value) {
                    ...
                    $.each(value.items, function(index, v) {
                         album += '<style> .foto{background-image: url("'+v.prefix+dimensions+v.suffix+'");}</style>';
                    });
                });
            ...
            search: function(){
                ...
                var location = "19.444053,-99.182342"; //london, shaftesbury avenue
            ...
            viewTrends: function(location){
                ...
                this.getJson(url, function(data){
                    ...
                    var setOfVenues = data.response.venues;
                    var template = '<div id="trendResults"></div>';
                    $('#lista').append(template);

                    $.each(setOfVenues, function(index, value) {
                       // console.log(value);
                        var of={'nombre':value.name,'lat':value.location.lat,'lon':value.location.lng};
                        pila.push(of);
                        if(index==3)
                            return false;
                        var innterListItem = '<div class="cont_list "><div class="titulo_l" ><div class="icono icon-tendencia_icn"></div><span class="t"><a class="trendlist" data-venueid="'+value.id+'" href="#">'+value.name+' </a></span></div></div> <div class="line"><img src="img/line.png"></div>';
